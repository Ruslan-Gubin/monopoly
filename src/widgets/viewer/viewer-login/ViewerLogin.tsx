...
const ViewerLogin = () => {
  ...
  return (
    <div className={styles.root}>
      ...
      <h2 onClick={loginStatusToggle} className={styles.header_link}>{loginStatus === 'login' ? "Войти" : "Зарегестрироватся"}</h2>
      </div>
    <form className={styles.form_container} onSubmit={(e) => validInputs(e, loginStatus, imag)}>
      {loginStatus !== "login" && (
        <>
          {imag.length > 0 ? (
            <picture>
